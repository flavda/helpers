...
#    pred = to_data(preds).max(1)[1] # get the index of the max log-probability
    pred = torch.max(to_data(preds), 1)[1]  # get the index of the max log-probability
...
def softmax_accuracy(preds, targets, size_average=True):
    ...
    print('++++++++++++++++++')
    print(reduction_fn(softmax_correct(preds,targets)))
...
def calculate_fid_from_generated_images_gpu(fid_model, model, data_loader,
    ...
    with torch.no_grad():
        ...
        for (data, labels), generated in zip(data_loader.test_loader, synthetic):
            data = Variable(data).cuda() if cuda else Variable(data)
            labels = Variable(labels).cuda() if cuda else Variable(labels)
...
def calculate_consistency_class(model, loader, reparam_type, vae_type, cuda=False):
    ...
    if model.current_model > 0 and (reparam_type == 'mixture'
        ...
        for img, labels in loader.test_loader:
            with torch.no_grad():
                img = Variable(img).cuda() if cuda else Variable(img)
                labels = Variable(labels).cuda() if cuda else Variable(labels)

                output_map = model(img, labels)
...
def estimate_fisher_class(model, data_loader, batch_size, sample_size=10000, cuda=False):
    ...
    for x, y in data_loader.train_loader:
        ...
        reconstr_x, params, y_hat_logits = model(x, y)
        loss = model.loss_function(reconstr_x, x, params, y_hat_logits, y)
